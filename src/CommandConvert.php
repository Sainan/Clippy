...
abstract class CommandConvert extends Command
{
	...
	static function instantiateIfMatches(string $in): ?static
	{
		...
		if(preg_match("/".self::REGEX_WORD_BEGIN."(?'in_amount'\d+) ?(?'in_unit'$units_regex) (in|to) ?(?'out_unit'$units_regex)".self::REGEX_WORD_END."/i", $in, $matches) === 1)
		{
			$in_amount = intval($matches["in_amount"]);
	...
	static function format(int|float $value, string $unit): string
	{
